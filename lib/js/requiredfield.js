		...
		if (dataType != null && dataType != "") {
			...
			switch (dataType) {
				...
				case "distance":
					...
					{
						applyRule("(([0-9]*\\.?[0-9]*) *')? *(([0-9]*\\.?[0-9]*) *\"?)? *");
						applyRule("([0-9]*\\.?[0-9]*) *((in(ches)?)|(ft)|(feet))")
					}
					if (dataRegion[0] == "METRIC" || dataRegion[0] == "")
					{
						applyRule("([0-9]*\\.?[0-9]*) *?[pnμmcdkMG]?m"); // picometers to gigameters
					}
					break;

				case "angle":
					dataTypeValid = false;
					applyRule("([0-9]*\\.?[0-9]*) *(°|(deg(rees)?)|(rad(ians)?)|(grad(ians)?))");
					break;

				case "email":
			44		// Don't think this validation is standards complient. But it at least appears not to be overly strict.
