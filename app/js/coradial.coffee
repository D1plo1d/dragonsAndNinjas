  ...
  $.extend $.ui.sketch.prototype,
    _create: ->
      ...
      this.$svg.delegate ".point", "beforemove", (event) =>
        ...
        # console.log this._pointToVector($(event.target))
        ...
        for point in constraint.points
          ...
          point_vector = unit_vector.multiply(radius)
          for i in [0..1]
            point_vector.elements[i] = parseFloat(point_vector.elements[i]) + parseFloat(center_vector.elements[i])

          #console.log "#{radius} #{center_vector.inspect()} || unit: #{unit_vector.inspect()} => #{point_vector.inspect()} || #{point_vector.elements[0]}, #{point_vector.elements[1]}"
